...
{
	private transient static HashMap jobPool;
	private transient static long count = 0;
	private int[][] board;
	...
	private Job instance(int depth)
	{
		...
		if(jobs == null || depth >= jobs.length)
		{
	 ...
	 */
	private Job init(Job parent, Move move) throws Exception
	{
		prune = false;
		...
		if(distance == 0)
		{
			JobManagerImpl.instance().addSolution(move.getParent());
			prune = true;
			return this;
		}
	...
	public void solve() throws Exception
	{
		...
		// execution queue
		if(depth == maxJobDepth && distance > FifteenPuzzle.grainSize)
		{
			JobManagerImpl.instance().put(deepcopy());
			return;
		}

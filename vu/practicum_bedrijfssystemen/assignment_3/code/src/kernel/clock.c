 ...
 *   SET_ALARMN:  a process wants to be alerted after a specified interval
 *                using a specified alarm
 *   SET_SYN_AL:  set the sync alarm
 *
 *
 * The input message is format m6.  The parameters are as follows:
 *
 *     m_type    CLOCK_PROC   FUNC    NEW_TIME   ALARM_NR
 * --------------------------------------------------------
 * | HARD_INT   |          |         |         |          |
 * |------------+----------+---------+---------+----------|
 * | GET_UPTIME |          |         |         |          |
 ...
 * |------------+----------+---------+---------+----------|
 * | SET_ALARMN | proc_nr  |f to call|  delta  | alarm_nr |
 * |------------+----------+---------+---------+----------|
...
PRIVATE int watchdog_proc[NR_ALARMS];	/* contains proc_nr at call of *watch_dog[]*/
PRIVATE watchdog_t watch_dog[NR_ALARMS][NR_TASKS+NR_PROCS];
...
FORWARD _PROTOTYPE( void common_setalarm, (int proc_nr,
		long delta_ticks, watchdog_t fuction, int alarm_nr) );
...
FORWARD _PROTOTYPE( void do_setalarmn, (message *m_ptr) );
 ...
 */
  ...
  while (TRUE) {
	...
	case SET_ALARMN: do_setalarmn(&mc);	break;
 ...
 */
  ...
  if (next_alarm <= realtime)
  {
    ...
    {
      int alarm_itr;
      int alarm_fired = 0;	/* alarm_fired is used to make sure only one
                                 * alarm fires at a time for a specific 
                                 * process
                                 */
      /* iterate through the alarm queues */
      for (alarm_itr = 0; alarm_itr < NR_ALARMS; alarm_itr++)
      {
        if (rp->p_alarm[alarm_itr] != 0)
        {
          /* See if this alarm time has been reached. */
          /* Skip if an alarm has already gone of for this process */
          if (rp->p_alarm[alarm_itr] <= realtime && !alarm_fired)
          {
            ...
            if (watch_dog[alarm_itr][proc_nr+NR_TASKS])
            {
              watchdog_proc[alarm_itr]= proc_nr;
              (*watch_dog[alarm_itr][proc_nr+NR_TASKS])();
            }
            else
            {
              switch (alarm_itr)
              {
                  ...
                  break;
                case 1:
                  cause_sig(proc_nr, SIGALRM2);
                  break;
                default:
                  printf("No signal defined for alarm %d\n", alarm_itr);
                  break;
              }
            }
            rp->p_alarm[alarm_itr] = 0;
            alarm_fired = 1;
          }

          /* Work on determining which alarm is next. */
          if (rp->p_alarm[alarm_itr] != 0 && rp->p_alarm[alarm_itr] < next_alarm) 
            next_alarm = rp->p_alarm[alarm_itr];
        }
      }
    }
  }
 ...
 * To remain compatible with existing applications do_setalarm adds the 
 * alarm_nr to the message and forwards it to do_setalarmn.
 */
  const int alarm_nr = 0;	/* do_setalarm always uses alarm 0 */
  m_ptr->ALARM_NR = alarm_nr;		
  do_setalarmn(m_ptr);
}
 ...
 *				do_setalarmn				     *
 *===========================================================================*/
PRIVATE void do_setalarmn(m_ptr)
message *m_ptr;			/* pointer to request message */
{
/* A process wants an alarm signal or a task wants a given watch_dog function
 * called after a specified interval.
 */
  ...
  int alarm_nr;			/* number of the alarm to modify */
  ...
  alarm_nr = m_ptr->ALARM_NR;           /* number of the alarm to modify */
  if (alarm_nr >= NR_ALARMS) 
    panic("Invalid alarm number received", alarm_nr);
  rp = proc_addr(proc_nr);
  mc.SECONDS_LEFT = (rp->p_alarm[alarm_nr] == 0 ? 0 : (rp->p_alarm[alarm_nr] - realtime + HZ-1)/HZ);
  if (!istaskp(rp)) function= 0;	/* user processes get signaled */
  common_setalarm(proc_nr, delta_ticks, function, alarm_nr);
}
 ...
 */
  ...
  int proc_nr;			/* which process wants the alarm */
  long delta_ticks;		/* in how many clock ticks does he want it? */
  const int alarm_nr = 0;	/* do_setsyn_alrm always uses alarm 0 */

  /* Extract the parameters from the message. */
  proc_nr = m_ptr->CLOCK_PROC_NR;	/* process to interrupt later */
  delta_ticks = m_ptr->DELTA_TICKS;	/* how many ticks to wait */
  rp = proc_addr(proc_nr);
  mc.SECONDS_LEFT = (rp->p_alarm[alarm_nr] == 0 ? 0 : (rp->p_alarm[alarm_nr] - realtime)/HZ );
  common_setalarm(proc_nr, delta_ticks, cause_alarm, alarm_nr);
}
 ...
 *				common_setalarm				     *
 *===========================================================================*/
PRIVATE void common_setalarm(proc_nr, delta_ticks, function, alarm_nr)
int proc_nr;			/* which process wants the alarm */
long delta_ticks;		/* in how many clock ticks does he want it? */
watchdog_t function;		/* function to call (0 if cause_sig is
				 * to be called */
int alarm_nr;			/* number of the alarm to be modified */
{
 ...
 */
  ...
  rp = proc_addr(proc_nr);
  rp->p_alarm[alarm_nr] = (delta_ticks == 0 ? 0 : realtime + delta_ticks);
  watch_dog[alarm_nr][proc_nr+NR_TASKS] = function;
 ...
 */
  ...
  syn_table[watchdog_proc[0] + NR_TASKS]= TRUE;

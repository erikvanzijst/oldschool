...
void handler(int sig)
{
  ...
  switch (sig)
  {
    case SIGALRM:
      ...
      printf("%s: Alarm 2 is rescheduled to arrive in %u seconds (%u seconds remaining)\n", timestr, seconds2, remaining);
      alarm_counter++;
      break;
    case SIGALRM2:
      printf("%s: Alarm 2 received\n", timestr);
      remaining = alarm(seconds1);
      printf("%s: Alarm 1 is rescheduled to arrive in %u seconds (%u seconds remaining)\n", timestr, seconds1, remaining);
      alarm_counter++;
      break;
    default:
...
int main(int argc, const char* argv[])
{
  ...
  act_alarm1.sa_flags &= ~SA_RESETHAND;
  act_alarm2.sa_handler = handler;
  act_alarm2.sa_flags &= ~SA_RESETHAND;

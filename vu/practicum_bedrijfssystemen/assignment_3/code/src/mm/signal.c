 ...
 *   do_alarm2:     perform the ALARM2 system call by calling set_alarm2()
 *   set_alarm2:    tell the clock task to start or stop the altenate timer
 *   set_alarmn:    tell the clock task to start or stop a specified timer
 */
...
FORWARD _PROTOTYPE( int set_alarmn, (int proc_nr, int sec, int alarm_nr));
 ...
 */
  ...
  for (i = 1; i <= _NSIG; i++) {
	...
	switch (i) {
		...
		break;
	    case SIGALRM2:
		/* Disregard SIGALRM2 when the target process has not
		 * requested an alarm.  This only applies for a KERNEL
		 * generated signal.
		 */
		if ((rmp->mp_flags & ALARM2_ON) == 0) continue;
		rmp->mp_flags &= ~ALARM2_ON;
		id = proc_id;
		break;
	    default:
		id = proc_id;
 ...
 *				do_alarm2				     *
 *===========================================================================*/
PUBLIC int do_alarm2()
{
/* Perform the alarm(seconds) system call. */

  return(set_alarm2(who, seconds));
}
 ...
 */
  ...
  return set_alarmn(proc_nr, sec, alarm_nr);
}
 ...
 *				set_alarm2				     *
 *===========================================================================*/
PUBLIC int set_alarm2(proc_nr, sec)
int proc_nr;			/* process that wants the alarm */
int sec;			/* how many seconds delay before the signal */
{
/* This routine is used by do_alarm2() to set the alternate alarm timer.  
 * It is also used to turn the timer off when a process exits with the timer 
 * still on.
 */

  message m_sig;
  int remaining;
  const int alarm_nr = 1;

  if (sec != 0)
    mproc[proc_nr].mp_flags |= ALARM2_ON;
  else
    mproc[proc_nr].mp_flags &= ~ALARM2_ON;
 ...
 *				set_alarmn				     *
 *===========================================================================*/
PRIVATE int set_alarmn(proc_nr, sec, alarm_nr)
int proc_nr;			/* process that wants the alarm */
int sec;			/* how many seconds delay before the signal */
int alarm_nr;			/* number of the alarm to modify */
{
/* This routine is used by set_alarm and set_alarm2 to set an alarm timer.  
 * It is also used to turn a timer off when a process exits with the timer 
 * still on.
 */

 ...
 *   SET_PROFILE: enable/disable profiling for the specified process
 *   GET_PROFILE: get the profile of the last profiled process
 *
 *
 ...
 *
 *     m_type      PROF_PROC   PROF_ADDR1   PROF_ADDR2   PROF_DATA
 * -----------------------------------------------------------------
 * | SET_PROFILE | proc pid  | start addr | end addr   |           |
 * |-------------+-----------+------------+------------|-----------|
 * | GET_PROFILE | proc nr   |            |            | profile   |
 * -----------------------------------------------------------------
 *
 * Reply messages are of type OK on success or contain an error code on 
 * failure. The GET_PROFILE function copies to profile to the pointer
 * supplied in PROF_DATA. PROF_PROC is the process number of the process 
 * holding this pointer. The SET_PROFILE function uses PROF_PROC to pass
 * the process id of the process to be profiled.
 */
...
/* Profiling variables */
#define PROF_BINS_MAX	1024

PRIVATE struct prof_data prof_data;	/* structure to hold details of the
                                           profiling task */
PRIVATE struct proc *prof_proc = NIL_PROC;	/* proc to be profiled */
PRIVATE int prof_enabled = 0;		/* indices profiling state */
PRIVATE int prof_bytes_per_bin = 0;	/* buffer for bytes per bin to
					   avoid recalculation on every
					   clock tick */
...
FORWARD _PROTOTYPE( int do_setprofile, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_getprofile, (message *m_ptr) );
FORWARD _PROTOTYPE( void prof_configure, (int start_addr, int end_addr) );
FORWARD _PROTOTYPE( void prof_count, (void) );
FORWARD _PROTOTYPE( struct proc *prof_findproc, (int pid) );
 ...
 */
  ...
  while (TRUE) {
     ...
     /* newly added functions change value of r, reset r to OK before 
      * calling any other function that does not set r 
      */
     r = OK;
     switch (opcode) {
	...
	case SET_PROFILE:	r = do_setprofile(&mc);	break;
	case GET_PROFILE:	r = do_getprofile(&mc);	break;
 ...
 */
  ...
  /* increment the bin counters if profiling is enabled  */
  if (prof_enabled) prof_count();
 ...
 *				do_setprofile				    *
 *==========================================================================*/
PRIVATE int do_setprofile(m_ptr)
message *m_ptr;			/* pointer to request message */
{
/* Verify arguments for validity and configure and enable the profiler to 
 * profile the process with the specified process id. Caliing do_setprofile 
 * with process id 0 disables profiling.
 */
  int r = OK;

  /* test arguments for validity */
  if(m_ptr->PROF_PROC < 0)
    return ESRCH;
  if (m_ptr->PROF_ADDR1 > m_ptr->PROF_ADDR2 ||
      m_ptr->PROF_ADDR1 < 0 ||
      m_ptr->PROF_ADDR2 < 0)
    return EFAULT;
  ...
  {
    /* not profiling, start if pid != 0 */
    if (m_ptr->PROF_PROC != 0)
    {
      prof_proc = prof_findproc(m_ptr->PROF_PROC);
      if (prof_proc == NIL_PROC) r = ESRCH;
      if (r == OK)
      {	
        int proc_end_addr;	/* buffer for the actual end address of 
                                 * the process
                                 */

        /* process with separate I&D and processes with combined I&D store 
         * their end address in a different location, store this address 
         * in the designated buffer  
         */
        if (prof_proc->p_map[T].mem_len != 0) 
        {
          /* separate I&D */
          proc_end_addr = (prof_proc->p_map[T].mem_len * CLICK_SIZE) - 1;
        }
        else
        {
          /* combined I&D */
          proc_end_addr = (prof_proc->p_map[D].mem_len * CLICK_SIZE) - 1;
        }

        /* If the end address is zero use the actual end address of the 
         * process; earlier checks force the start address to be zero if the 
         * end address is zero so we do not need to modify the start address.
         * If the end address is non-zero it should lie within the range of 
         * the program. In earlier checks we already made sure the adresses
         * are non-negative and that the start address is not larger than
         * the end address so we know for sure that if the end address is
         * in range, the start address is in range as well.
         */
        if (m_ptr->PROF_ADDR2 == 0)
        {
          /* auto configure address range */
          m_ptr->PROF_ADDR2 = proc_end_addr;
        }
        else
        {
          /* check address range */
          if (m_ptr->PROF_ADDR2 > proc_end_addr) r = EFAULT;
        }
        if (r == OK)
        {
      	  prof_configure(m_ptr->PROF_ADDR1, m_ptr->PROF_ADDR2);
      	  prof_enabled = 1;
      	}
      }
    }
 ...
 *				get_profile				    *
 *==========================================================================*/
PRIVATE int do_getprofile(m_ptr)
message *m_ptr;			/* pointer to request message */
{
/* Copy the profile of the last profiled process to the pointer supplied by
 * the process identified by process number PROF_PROC.
 */
  int r = OK;
  ...
  lock();
  /* determine the physical address of the user space profile data area */
  user_phys = numap(m_ptr->PROF_PROC, (vir_bytes) m_ptr->PROF_DATA, sizeof(prof_data));
  if (!user_phys) r = EFAULT;
  if (r == OK)
  {
    /* determine the physical address of the kernel space profile data area */
    kernel_phys = vir2phys(&prof_data);

    /* copy the profile data from kernel space to user space */
    phys_copy(kernel_phys, user_phys, (phys_bytes) sizeof(prof_data));
  }
  unlock();
 ...
 *				prof_configure				    *
 *==========================================================================*/
PRIVATE void prof_configure(int start_addr, int end_addr)
{
/* Configure the profiler by calculating the number of bytes per bin and 
 * the number of bins to use for the profile of the specified memory area.
 */
  int bytes;
  int bin_itr;

  /* set addresses */
  prof_data.start_addr = start_addr;
  prof_data.end_addr = end_addr;

  /* calculate bins_used here */
  /* both addresses are inclusive, add 1 */
  bytes = (prof_data.end_addr - prof_data.start_addr) + 1;
  prof_bytes_per_bin = bytes / PROF_BINS_MAX;

  /* if bytes divided by PROF_BINS_MAX is not an integer add one 
   * to prof_bytes_per_bin to make address range fit in PROF_BINS_MAX 
   */
  if (bytes % PROF_BINS_MAX != 0) prof_bytes_per_bin++;

  if (prof_bytes_per_bin != 0) 
  {
    /* if prof_bytes_per_bin is non_zero there exists a 
     * prof_data.bins_used 
     */
    prof_data.bins_used = bytes / prof_bytes_per_bin;
    if (bytes % prof_bytes_per_bin != 0) prof_data.bins_used++;

    /* reset bins  */
    for (bin_itr = 0; bin_itr < PROF_BINS_MAX; bin_itr++)
    {
      prof_data.buffer[bin_itr] = 0;
    }
  }
  else
  {
    /* prof_bytes_per_bin is zero, nothing to profile, 
     * prof_data.bins_used = 0 
     */
    prof_data.bins_used = 0;
  }
}
 ...
 *				prof_count				    *
 *==========================================================================*/
PRIVATE void prof_count(void) 
{
/* Increment the appropriate bin if the program counter is in the specified
 * memory range. If both start address and end address are zero determine
 * the correct end address first.
 */
  int bin;
  int cur_addr = prof_proc->p_reg.pc;

  if (prof_proc->p_flags == 0) 
  {
    /* process is runnable */
    if (prof_data.start_addr <= cur_addr && cur_addr <= prof_data.end_addr)
    {
      /* current address in profiling range */
      if (prof_bytes_per_bin != 0)
      {
        /* if prof_bytes_per_bin is non_zero there exists a bin */
        bin = (cur_addr - prof_data.start_addr) / prof_bytes_per_bin;
        /* increment the bin number */
        prof_data.buffer[bin]++;
      }
      /* end of current address in profiling range */
    }
    /* end of process is runnable */
  }
  else if (prof_proc->p_flags & P_SLOT_FREE) 
  {
    /* slot no longer in use, disable profiling */
    prof_enabled = 0;
  }
}
 ...
 *				prof_findproc				    *
 *==========================================================================*/
PRIVATE struct proc *prof_findproc(int pid)
{
/* Determine the process number of the process with the specified process 
 * id.
 */
  struct proc *rp;

  for (rp = BEG_PROC_ADDR; rp < END_PROC_ADDR; rp++)
    if (!(rp->p_flags & P_SLOT_FREE) && rp->p_pid == pid) 
      return rp;
  return NIL_PROC;
}

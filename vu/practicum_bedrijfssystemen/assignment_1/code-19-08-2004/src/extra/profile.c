 ...
 *
 * 17.aug.2004
 * The profiler is now synchronized with the start of the target application
 * using ptrace().
 *
...
int main(int argc, const char* argv[])
{
  ...
  /* prepare to handle to user signal if the child process dies unexpected
   */
  ...
  {
    /* enable tracing of the profiled process to enable synchronization with
     * exec call */
    if (ptrace(T_OK, 0, 0, 0) == -1)
    {
      /* ptrace failed */
      int retval = errno;
      kill(getppid(), SIGUSR1); /* notify parent that we failed */
      exit(retval);
    }
  ...
  else	
  {
    ...
    /* wait for the child to reach the synchronization point set by ptrace
     * call 
     */
    if(wait(&status) == -1)
    {
    ...
    {
      /* child failed to stop on trace point */
      fprintf(stderr, "Failed to synchronize with child\n");
      exit(EXIT_FAILURE);
    }
    
    /* trace point reached, enable the profiler */
    ...
    do
    {
      /* the profiler is enabled, we do no longer want to trace the child */
      if (ptrace(T_RESUME, pid, 0, 0) == -1)
      {
        /* ptrace failed */
        perror("Error resuming child process");
        /* kill the child */
        kill(pid, SIGKILL);
        /* wait for the child */
        wait(0);
        /* disable the profiler so it can be used for a new run */
        profile(0, 0, 0);	
        exit(EXIT_FAILURE);
      }
  
      /* the profiler was sucessfully enabled, wait for the child 
       * to terminate 
       */
      if(wait(&status) == -1)
      {
    ...
    while (WIFEXITED(status) == 0 && WIFSTOPPED(status));

    if (WIFEXITED(status) == 0)
    {
      /* child exited abnormally */
      fprintf(stderr, "Child executed abnormally\n");
      /* disable the profiler so it can be used for a new run */
      profile(0, 0, 0);	
      exit(EXIT_FAILURE);
    }
...
int parse_args(int argc, const char* argv[], config_t *config)
{
  ...
  for(arg_itr = 1; arg_itr < argc && !retval; arg_itr++)
  {
    ...
    if(!prog_arg_itr && strcmp(argv[arg_itr], "-n") == 0)
    {
      ...
      {
        /* value available */
        const int bar_count_base = 10;
        char *endptr;
        config->bar_count = strtol(argv[++arg_itr], &endptr, bar_count_base);
        if(endptr[0] != 0 || config->bar_count < 1)
        {
          /* value contains non-numeric characters or value is too
           * low; at least one row should be displayed 
           */
          retval = EINVAL;
        }
      }
    ...
    else if(!prog_arg_itr && strcmp(argv[arg_itr], "-r") == 0)
    {
      ...
      if(arg_itr < (argc - 1))
      {
        ...
        if(delim_pos >= 1 && strlen(range) - (delim_pos + 1) >= 1)
        {
          ...
          config->start_addr = strtol(range, &endptr, address_base);
          if (endptr[0] == delimiter)
          {
            /* start address ends at the delimiter */
            config->end_addr = strtol(&range[delim_pos + 1], &endptr, address_base);
            if (endptr[0] != 0)
            {
              /* non-numeric data in end address */
              retval = EINVAL;
            }
          }
          else
          {
            /* start address contains non-numeric data */
            retval = EINVAL;
          }
        }
        else
    ...
    else
    {
      do
      {
        ...
        if (result)
        {
          ...
          config->program_args[++prog_arg_itr] = '\0';
        }
        else
        {
      ...
      while (arg_itr + prog_arg_itr < argc && !retval);
      arg_itr += prog_arg_itr;
    }
  }
...
void signal_handler(int signal)
{
  ...
  switch (signal)
  {
    case SIGUSR1:
      ...
      if (wait(&status) == -1)
      {
      	perror("Failed to retrieve status from child process");
        exit(EXIT_FAILURE);
      }
      if (WIFEXITED(status))
      {
      ...
      break;
    default:
      fprintf(stderr, "Unexpected signal received from child\n");
      exit(EXIT_FAILURE);
      break;
  }
}

...
int bank_pay(char *bank_ip_address, int bank_port_nb, int source_account, int destination_account, float amount, char *password)
{
  ...
  msg.amount = amount;
  if (password) strlcpy(msg.passwd, password, sizeof(msg.passwd));
  else memset(msg.passwd, 0, sizeof(msg.passwd));
...
int bank_check(char *bank_ip_address, int bank_port_nb, int source_account, int destination_account, float amount, int certificate, char *password)
{
  struct protocol_msg msg;
  char buffer[PROTOCOL_BUFFER_SIZE];
  int server_fd = -1;

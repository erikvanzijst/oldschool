...
IF SPATIE = 0 THEN PLAY "P64"
IF KEY$ = CHR$(0) + CHR$(72) THEN GOSUB REFRESH: R = R + 1: RC = 1
IF R > 4 THEN R = 1                              'R is de Rotatieconstante
BLOK6 XB%, YB%, R, V, RC
V = 0
RC = 0
IF YB% = 40 THEN SOORTBLOK = PREVIEW: RETURN
...
IF KEY$ = CHR$(80) OR KEY$ = CHR$(112) THEN PAUZE: GOSUB NAPAUZE
IF KEY$ = CHR$(27) THEN sluitscherm HISCORE: IF HISCORE = 1 THEN GOTO 200
NEXT I
...
LINE (-30, 160)-(230, 250), 0, BF       'Hier wordt gekeken, of de score hoog
LINE (-30, 160)-(230, 250), 13, B       'genoeg is, om in de Highscore te ko-
LOCATE 16, 37: COLOR 15: PRINT "GAME OVER": GOTO 300
200
LINE (-30, 160)-(230, 250), 0, BF
LINE (-30, 160)-(230, 250), 13, B
...
LOCATE 12, 58: COLOR 15: PRINT "ÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ"
END SUB
